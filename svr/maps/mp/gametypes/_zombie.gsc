...
bot_patrol_waypoints(){
	// Initialize variables if not set
	if (!isDefined(self.lastOrigin))
		self.lastOrigin = self.origin;
	if (!isDefined(self.stuckTime))
		self.stuckTime = getTime();
	if (!isDefined(self.stuckCount))
		self.stuckCount = 0;
	if (!isDefined(self.waypointStuckTime))
		self.waypointStuckTime = getTime();
	if (!isDefined(self.offPathTime))
		self.offPathTime = getTime();
	if (!isDefined(self.offPathCount))
		self.offPathCount = 0;
		
	eye = self.origin + (0, 0, 60);
	
	// Initialize waypoint history if needed
	if(!isDefined(self.lastPosition))
		self.lastPosition = self.origin;
	if(!isDefined(self.stuckStartTime))
		self.stuckStartTime = getTime();
	
	// Check if we're completely off-path (not near any waypoint)
	closestWaypointDist = 99999;
	...
	// If we're too far from any waypoint, we're off-path
	if (closestWaypointDist > 200)
	{
		if (!isDefined(self.offPathTime))
			self.offPathTime = getTime();
			
		if (getTime() - self.offPathTime > 2000) // 2 seconds off-path
		{
			self.offPathCount++;
			self.offPathTime = getTime();
			
			// If we've been off-path for too long, teleport to nearest waypoint
			if (self.offPathCount > 3)
			{
				if (closestWaypointIndex >= 0)
				{
					self setOrigin(level.waypoints[closestWaypointIndex].origin);
			...
			// Try to move towards the closest waypoint
			if (closestWaypointIndex >= 0)
			{
				targetOrigin = level.waypoints[closestWaypointIndex].origin;
				...
				trace = bulletTrace(eye, targetOrigin + (0,0,60), false, self);
				
				if (trace["fraction"] >= 0.3) // Even partially clear path
				{
				...
				{
					// Try to find a clear direction towards the waypoint
					for(angle = -90; angle <= 90; angle += 30)
					{
						testAngles = (0, targetDirection[1] + angle, 0);
						testForward = anglesToForward(testAngles);
						testTrace = bulletTrace(eye, eye + (testForward[0] * 100, testForward[1] * 100, testForward[2] * 100), false, self);
						
						if (testTrace["fraction"] >= 0.5)
						{
							self setPlayerAngles(testAngles);
							self setWalkDir("forward");
							return;
						}
					}
	...
	{
		// Reset off-path counters when we're near a waypoint
		self.offPathCount = 0;
		self.offPathTime = getTime();
	}
	
	// Check if we're stuck by comparing position
	dist = distance(self.origin, self.lastPosition);
	if(dist < 10) // If barely moved
	{
		if(getTime() - self.stuckStartTime > 2000) // Stuck for 2 seconds
		{
			self.currentWaypoint = undefined;
			self.prevWaypoints = []; // Clear waypoint history when stuck
			self.stuckStartTime = getTime();
		}
	}
	else
	...
	// Check if we're at current waypoint for too long
	if (isDefined(self.currentWaypoint))
	{
		if (getTime() - self.waypointStuckTime > 3000) // 3 seconds max per waypoint
		{
			self.currentWaypoint = undefined;
			self.waypointStuckTime = getTime();
		}
	...
	// If we don't have a waypoint or current one is invalid/blocked, find a new one
	if (!isDefined(self.currentWaypoint) || !isDefined(level.waypoints[self.currentWaypoint]))
	{
		// Only use findBestWaypoint for random patrolling, not for enemy-directed movement
		if (!isDefined(self.usingEnemyWaypoint) || !self.usingEnemyWaypoint)
		{
			self.currentWaypoint = self findBestWaypoint();
			if (isDefined(self.currentWaypoint))
				self.waypointStuckTime = getTime();
		}
		else
		{
			// If we were using enemy waypoint but it's invalid, clear the flag and find random waypoint
			self.usingEnemyWaypoint = false;
			self.currentWaypoint = self findBestWaypoint();
			if (isDefined(self.currentWaypoint))
				self.waypointStuckTime = getTime();
		}
			...
			else 
	{
		...
		if (isDefined(self.currentWaypoint) && isDefined(level.waypoints[self.currentWaypoint]))
		{
			currentWPOrigin = level.waypoints[self.currentWaypoint].origin;
			directTrace = bulletTrace(eye, currentWPOrigin + (0,0,60), false, self);
			dist = distance(self.origin, currentWPOrigin);
			
			// If waypoint is blocked or too far, find a new one
			if (directTrace["fraction"] < 0.6 || dist > 300)
			{
				// Only use findBestWaypoint for random patrolling
				if (!isDefined(self.usingEnemyWaypoint) || !self.usingEnemyWaypoint)
				{
					self.currentWaypoint = self findBestWaypoint();
				}
				...
				{
					// If we were using enemy waypoint but it's blocked, clear the flag
					self.usingEnemyWaypoint = false;
					self.currentWaypoint = self findBestWaypoint();
				}
			}
		...
		{
			// If current waypoint is invalid, find a new one
			if (!isDefined(self.usingEnemyWaypoint) || !self.usingEnemyWaypoint)
			{
				self.currentWaypoint = self findBestWaypoint();
			}
			...
			{
				// If we were using enemy waypoint but it's invalid, clear the flag
				self.usingEnemyWaypoint = false;
				self.currentWaypoint = self findBestWaypoint();
			}
		}
	...
	if (isDefined(self.currentWaypoint) && isDefined(level.waypoints[self.currentWaypoint]))
	{
		targetOrigin = level.waypoints[self.currentWaypoint].origin;
		dir = targetOrigin - self.origin;
		dist = distance(self.origin, targetOrigin);
		
		// Calculate target direction and turn bot towards waypoint
		targetDirection = vectorToAngles(vectorNormalize(dir));
		self setPlayerAngles((0, targetDirection[1], 0));
		
		if (!isDefined(self.lastMoveDir))
			self.lastMoveDir = "none";
			
		if (!isDefined(self.dirChangeCount))
			self.dirChangeCount = 0;
			
		// Check for obstacles in front
		forward = anglesToForward(self getPlayerAngles());
		traceForward = bulletTrace(eye, eye + (forward[0] * 50, forward[1] * 50, forward[2] * 50), false, self);
		
		if (traceForward["fraction"] >= 0.6)
		{
			self setWalkDir("forward");
			self.lastMoveDir = "forward";
			self.lastOrigin = self.origin;
			self.stuckTime = getTime();
			self.stuckCount = 0;
			self.dirChangeCount = 0;
		...
		{
			// Try to navigate around obstacle by strafing
			right = anglesToRight(self getPlayerAngles());
			
			traceLeft = bulletTrace(eye, eye + (right[0] * -50, right[1] * -50, right[2] * -50), false, self);
			traceRight = bulletTrace(eye, eye + (right[0] * 50, right[1] * 50, right[2] * 50), false, self);
			
			// If we've been changing directions too much, try a new waypoint
			if (self.dirChangeCount > 3)
			{
				self.currentWaypoint = self findBestWaypoint();
				self.dirChangeCount = 0;
				return;
		...
		if (distance(self.origin, self.lastOrigin) < 10)
		{
			if (getTime() - self.stuckTime > 1.0)
			{
				self.stuckCount++;
				if (self.stuckCount > 3)
				{
					// Back up and find new waypoint if stuck
					back = anglesToForward(self getPlayerAngles());
					traceBack = bulletTrace(eye, eye - (back[0] * 50, back[1] * 50, back[2] * 50), false, self);
					
					if (traceBack["fraction"] >= 0.6)
					{
		...
		if (distance(self.origin, targetOrigin) < 50)
		{
			// Clear enemy waypoint flag when we reach the waypoint
			if (isDefined(self.usingEnemyWaypoint) && self.usingEnemyWaypoint)
			{
				self.usingEnemyWaypoint = false;
			}
	...
	// Get normalized direction vectors based on current angles
	angles = self getPlayerAngles();
	forward = vectorNormalize(anglesToForward(angles));
	right = vectorNormalize(anglesToRight(angles));
	
	vecForward = forward;
	vecLeft = vectorNormalize((right[0] * -1, right[1] * -1, right[2] * -1));
	vecRight = right;
	vecBack = vectorNormalize((forward[0] * -1, forward[1] * -1, forward[2] * -1));
	
	// Check multiple heights and distances for forward movement
	shortDist = 40;  // Very close check
	midDist = 80;    // Medium distance check
	longDist = 120;  // Long distance check
	
	// Multiple trace checks at different heights and distances
	traceForwardLow = bulletTrace(eye - (0,0,40), eye - (0,0,40) + (vecForward[0] * midDist, vecForward[1] * midDist, vecForward[2] * midDist), false, self);
	traceForwardMid = bulletTrace(eye, eye + (vecForward[0] * midDist, vecForward[1] * midDist, vecForward[2] * midDist), false, self);
	traceForwardHigh = bulletTrace(eye + (0,0,40), eye + (0,0,40) + (vecForward[0] * midDist, vecForward[1] * midDist, vecForward[2] * midDist), false, self);
	
	// Check very close for immediate obstacles
	traceVeryClose = bulletTrace(eye, eye + (vecForward[0] * shortDist, vecForward[1] * shortDist, vecForward[2] * shortDist), false, self);
	
	// If path is clear at any height and no immediate obstacle, move forward
	if ((traceForwardLow["fraction"] >= 0.9 || traceForwardMid["fraction"] >= 0.9 || traceForwardHigh["fraction"] >= 0.9) && traceVeryClose["fraction"] > 0.5)
	{
		self setWalkDir("forward");
		self.lastOrigin = self.origin;
		self.stuckTime = getTime();
	...
	if (distance(self.origin, self.lastOrigin) < 20)
	{
		if (!isDefined(self.lastStuckTime))
			self.lastStuckTime = getTime();
			
		// Increment stuck counter if we've been stuck for a bit
		if (getTime() - self.lastStuckTime > 500)  // 0.5 seconds
		{
			self.stuckCount++;
			self.lastStuckTime = getTime();
		}
		
		// More aggressive unstuck behavior
		if (self.stuckCount >= 2)
		{
			// Try a series of movements to get unstuck
			self setWalkDir("back");
			wait 0.2;
			...
			{
				self setPlayerAngles((0, self getPlayerAngles()[1] + 45, 0));
				self setWalkDir("right");
			}
			else
			{
			...
			// Pick new waypoint if still stuck
		...
		// Check wider angles for potential paths
		for(angle = 45; angle <= 315; angle += 45)
		{
			checkAngles = (0, self getPlayerAngles()[1] + angle, 0);
			checkForward = anglesToForward(checkAngles);
			tracePath = bulletTrace(eye, eye + (checkForward[0] * midDist, checkForward[1] * midDist, checkForward[2] * midDist), false, self);
			
			if (tracePath["fraction"] >= 0.8)
			{
				self setPlayerAngles(checkAngles);
				self setWalkDir("forward");
				return;
			}
		}
		
		// If no good path found, try side movement
		traceSidesLeft = bulletTrace(eye, eye + (vecLeft[0] * shortDist, vecLeft[1] * shortDist, vecLeft[2] * shortDist), false, self);
		traceSidesRight = bulletTrace(eye, eye + (vecRight[0] * shortDist, vecRight[1] * shortDist, vecRight[2] * shortDist), false, self);
		
		if (traceSidesLeft["fraction"] >= 0.7)
		{
			self setWalkDir("left");
			wait 0.2;
			self setWalkDir("forward");
		...
		if (traceSidesRight["fraction"] >= 0.7)
		{
			self setWalkDir("right");
			wait 0.2;
			self setWalkDir("forward");
		...
		// Try going back if sides are blocked
		traceBack = bulletTrace(eye, eye + (vecBack[0] * 100, vecBack[1] * 100, vecBack[2] * 100), false, self);
		if (traceBack["fraction"] >= 1.0)
		{
			// Turn 180 degrees from current angle
			newAngles = self getPlayerAngles();
			newAngles = (newAngles[0], newAngles[1] + 180, newAngles[2]);
		...
		// If all directions are blocked, pick new waypoint
		self.currentWaypoint = randomint(level.waypoints.size);
	}
	
	// All directions blocked, pick new waypoint
	self.currentWaypoint = randomint(level.waypoints.size);
	self.stuckTime = getTime();
	
	if (self.lastRotation == 0)
		vec = (1, 0, 0);    // Forward
	else if (self.lastRotation == 1)
		vec = (0, 1, 0);    // Right
	else if (self.lastRotation == 2)
		vec = (-1, 0, 0);   // Back
	else
		vec = (0, -1, 0);   // Left
		
	traceEnd = (eye[0] + vec[0] * 100, eye[1] + vec[1] * 100, eye[2] + vec[2] * 100);
	trace = bulletTrace(eye, traceEnd, false, self);
	
	if (trace["fraction"] >= 1.0)
	{
		// Path is clear, move in that direction
		if (self.lastRotation == 0)
			self setWalkDir("forward");
		else if (self.lastRotation == 1)
			self setWalkDir("right");
		...
		// Try right
		vecRight = (0, 1, 0);
		traceEndRight = (eye[0] + vecRight[0] * 100, eye[1] + vecRight[1] * 100, eye[2] + vecRight[2] * 100);
		traceRight = bulletTrace(eye, traceEndRight, false, self);
		if (traceRight["fraction"] >= 1.0)
		{
			self setWalkDir("right");
			self.lastWalkDir = "right";
			self.avoidTime = undefined;
		}
		else {
			// Try back
			vecBack = (-1, 0, 0);
			traceEndBack = (eye[0] + vecBack[0] * 100, eye[1] + vecBack[1] * 100, eye[2] + vecBack[2] * 100);
			traceBack = bulletTrace(eye, traceEndBack, false, self);
			if (traceBack["fraction"] >= 1.0)
			{
				self setWalkDir("back");
				self.lastWalkDir = "back";
				self.avoidTime = undefined;
				// After going back, pick a new waypoint to avoid looping
				self.currentWaypoint = randomint(level.waypoints.size);
			}
			else {
				// All directions blocked, pick a new waypoint
				self.currentWaypoint = randomint(level.waypoints.size);
			}
		}
	}
	// Do not return here: allow stuck/waypoint logic to run
	// Stuck detection and strafe logic
	if (!isDefined(self.lastOrigin))
		self.lastOrigin = self.origin;
	if (!isDefined(self.stuckTime))
		self.stuckTime = getTime();
	...
	if (distance(self.origin, self.lastOrigin) < 10)
	{
		if (getTime() - self.stuckTime > 1.0)
		{
			...
			// Randomly turn left or right to get unstuck
			if (randomfloat(1) < 0.5)
				self setWalkDir("left");
			else
				self setWalkDir("right");
	...
	// Debug print for waypoint 86
	if (isDefined(level.waypoints) && level.waypoints.size > 86 && isDefined(level.waypoints[86]) && isDefined(level.waypoints[86].origin))
	{
		iprintlnbold("WP86: " + level.waypoints[86].origin);
	}
	...
	// If no current waypoint, pick closest
	if (!isDefined(self.currentWaypoint))
	{
		minDist = 999999;
		for (i = 0; i < level.waypoints.size; i++)
		{
			...
			if (!isDefined(wp) || !isDefined(wp.origin))
				continue;
			dist = distance(self.origin, wp.origin);
			if (dist < minDist)
			{
				minDist = dist;
				self.currentWaypoint = i;
			}
		}
	}
...
// Manual absolute value logic for direction
x = dir[0];
y = dir[1];
if (x < 0)
	absx = 0 - x;
...
// Prioritize directions: calculated, forward, right, left, back
eye = self.origin + (0, 0, 60);
directionList = [];
if (absx > absy)
{
	...
	{
		directionList = [];
		directionList[0] = "forward";
		directionList[1] = "right";
		directionList[2] = "left";
		directionList[3] = "back";
	}
	else
	{
		directionList = [];
		directionList[0] = "back";
		directionList[1] = "right";
		directionList[2] = "left";
		directionList[3] = "forward";
	}
}
else
{
	...
	{
		directionList = [];
		directionList[0] = "right";
		directionList[1] = "forward";
		directionList[2] = "back";
		directionList[3] = "left";
	}
	else
	{
		directionList = [];
		...
		directionList[1] = "forward";
		directionList[2] = "back";
		directionList[3] = "right";
	}
}

// Avoidance logic: if recently blocked, keep sidestepping
if (isDefined(self.avoidTime) && getTime() < self.avoidTime)
{
	// Continue sidestepping
	self setWalkDir(self.avoidDir);
	self.lastWalkDir = self.avoidDir;
	return;
}

// Calculate direction to waypoint
targetVector = (targetOrigin - self.origin);
targetVector = (targetVector[0], targetVector[1], 0); // Ignore Z difference
length = (targetVector[0] * targetVector[0] + targetVector[1] * targetVector[1]);
if (length > 0)
{
    // Since we can't use sqrt, we'll normalize approximately
    ...
    targetVector = (targetVector[0] * scale, targetVector[1] * scale, 0);
}

// Convert to local space forward vector
vec = targetVector;
traceEnd = (eye[0] + vec[0] * 100, eye[1] + vec[1] * 100, eye[2] + vec[2] * 100);
trace = bulletTrace(eye, traceEnd, false, self);
if (trace["fraction"] >= 1.0)
{
	self setWalkDir("forward");
	self.lastWalkDir = "forward";
	self.avoidTime = undefined;
	return;
}

// If forward blocked, try back
vecBack = (-1, 0, 0);
traceEndBack = (eye[0] + vecBack[0] * 100, eye[1] + vecBack[1] * 100, eye[2] + vecBack[2] * 100);
traceBack = bulletTrace(eye, traceEndBack, false, self);
...
if (traceBack["fraction"] >= 1.0)
{
	...
	self.lastWalkDir = "back";
	self.avoidDir = "back";
	self.avoidTime = getTime() + 1.0;
	// After going back, pick a new waypoint to avoid looping
	self.currentWaypoint = randomint(level.waypoints.size);
	return;
}

// If back is blocked, try left and right
vecLeft = (0, -1, 0);
traceEndLeft = (eye[0] + vecLeft[0] * 100, eye[1] + vecLeft[1] * 100, eye[2] + vecLeft[2] * 100);
traceLeft = bulletTrace(eye, traceEndLeft, false, self);

vecRight = (0, 1, 0);
traceEndRight = (eye[0] + vecRight[0] * 100, eye[1] + vecRight[1] * 100, eye[2] + vecRight[2] * 100);
traceRight = bulletTrace(eye, traceEndRight, false, self);

if (traceLeft["fraction"] >= 1.0 && traceRight["fraction"] >= 1.0)
{
	// Both open, pick randomly
	if (randomfloat(1) < 0.5)
	{
		self setWalkDir("left");
		self.lastWalkDir = "left";
		self.avoidDir = "left";
	}
	else
	{
	...
	self.avoidTime = getTime() + 1.0; // Sidestep for 1 second
	return;
}
else if (traceLeft["fraction"] >= 1.0)
{
	self setWalkDir("left");
	self.lastWalkDir = "left";
	self.avoidDir = "left";
...
else if (traceRight["fraction"] >= 1.0)
{
	self setWalkDir("right");
	self.lastWalkDir = "right";
	self.avoidDir = "right";
...
return;
	...
	// Look ahead for wall detection AFTER setting walk direction
	eye = self.origin + (0, 0, 60); // Approximate eye height
	forward = (0, 0, 0);
	if (isDefined(self.lastWalkDir))
	{
		if (self.lastWalkDir == "forward")
			forward = (1, 0, 0);
		else if (self.lastWalkDir == "back")
			forward = (-1, 0, 0);
		else if (self.lastWalkDir == "right")
			forward = (0, 1, 0);
		else if (self.lastWalkDir == "left")
			forward = (0, -1, 0);
	}
	else
	{
		forward = (1, 0, 0); // Default forward
	}
	traceEnd = (eye[0] + forward[0] * 100, eye[1] + forward[1] * 100, eye[2] + forward[2] * 100);
	trace = bulletTrace(eye, traceEnd, false, self);
	if (trace["fraction"] < 1.0)
	{
		// Wall detected ahead, try to go back if not already
		if (self.lastWalkDir == "left" || self.lastWalkDir == "right")
		{
			// Try back
			eyeBack = self.origin + (0, 0, 60);
			backVec = (-1, 0, 0);
			traceEndBack = (eyeBack[0] + backVec[0] * 100, eyeBack[1] + backVec[1] * 100, eyeBack[2] + backVec[2] * 100);
			traceBack = bulletTrace(eyeBack, traceEndBack, false, self);
			if (traceBack["fraction"] >= 1.0)
			{
				self setWalkDir("back");
				self.lastWalkDir = "back";
		...
		// If already tried back or all blocked, pick a new waypoint
		self.currentWaypoint = randomint(level.waypoints.size);
		return;
	}

	// If close to waypoint, pick a new random one
	if (distance(self.origin, targetOrigin) < 100)
	{
		self.currentWaypoint = randomint(level.waypoints.size);
	}
}
...
findBestWaypoint()
{
	...
	// Track our previous waypoints to prevent loops
	if(!isDefined(self.prevWaypoints))
		self.prevWaypoints = [];
	
	// First try connected waypoints if we have a current one
	if(isDefined(self.currentWaypoint) && isDefined(level.waypoints[self.currentWaypoint]))
	{
		current = level.waypoints[self.currentWaypoint];
		for(c = 0; c < current.childCount; c++)
		{
			child = current.children[c];
			if(!isDefined(level.waypoints[child]))
				continue;
				
			// Skip if we've visited this waypoint recently (but be less strict)
			alreadyVisited = false;
			for(p = 0; p < self.prevWaypoints.size; p++)
			{
				if(self.prevWaypoints[p] == child)
				{
					alreadyVisited = true;
					break;
				}
			...
			trace = bullettrace(eye, level.waypoints[child].origin, false, self);
			if(trace["fraction"] > 0.7) // Less strict line of sight requirement
			{
				dist = distance(self.origin, level.waypoints[child].origin);
				if(dist < bestDist)
				{
					bestDist = dist;
					bestWP = child;
	...
	// If no connected waypoint found, look for any reachable waypoint
	if(!isDefined(bestWP))
	{
		for(i = 0; i < level.waypoints.size; i++)
		{
			...
			// Skip recently visited waypoints (but be even less strict)
			alreadyVisited = false;
			for(p = 0; p < self.prevWaypoints.size; p++)
			{
				if(self.prevWaypoints[p] == i)
				{
					alreadyVisited = true;
					break;
				}
			...
			wpOrigin = level.waypoints[i].origin;
			dist = distance(self.origin, wpOrigin);
			if(dist > 800) // Even larger range to explore more of the map
			continue;
			
		// Check if we have clear line of sight
		trace = bulletTrace(eye, wpOrigin + (0,0,60), false, self);
		
		// Prefer closer waypoints with clear paths - be more strict about walls
			if(trace["fraction"] > 0.7 && dist < bestDist) // More strict path requirement
		{
			bestDist = dist;
			bestWP = i;
			}
	...
	// If still no waypoint found, try any waypoint regardless of history
	if(!isDefined(bestWP))
	{
		for(i = 0; i < level.waypoints.size; i++)
		{
			...
			wpOrigin = level.waypoints[i].origin;
			dist = distance(self.origin, wpOrigin);
			if(dist > 800) // Even larger range for fallback
				continue;
			
			// Check if we have any path (more strict)
			trace = bulletTrace(eye, wpOrigin + (0,0,60), false, self);
			
			if(trace["fraction"] > 0.6 && dist < bestDist) // More strict path requirement
			{
				bestDist = dist;
				bestWP = i;
			}
	...
	// If we found a good waypoint, update history and use it
	if(isDefined(bestWP) && bestDist < 999999)
	{
		// Update waypoint history
		if(!isDefined(self.prevWaypoints))
			self.prevWaypoints = [];
			
			self.prevWaypoints[self.prevWaypoints.size] = bestWP;
		if(self.prevWaypoints.size > 3) // Reduced history size to allow more revisiting
			{
			// Manual array removal - shift all elements left
				for(i = 1; i < self.prevWaypoints.size; i++)
					self.prevWaypoints[i-1] = self.prevWaypoints[i];
			// Set the last element to undefined to effectively reduce size
				self.prevWaypoints[self.prevWaypoints.size-1] = undefined;
			}
				
			return bestWP;
	}
	
	// If no good waypoints found, pick a random unvisited one
	potentialWPs = [];
	for(i = 0; i < level.waypoints.size; i++)
	{
		if(!isDefined(level.waypoints[i]) || !isDefined(level.waypoints[i].origin))
		...
		// Skip recently visited waypoints (but be less strict)
		alreadyVisited = false;
		for(p = 0; p < self.prevWaypoints.size; p++)
		{
			if(self.prevWaypoints[p] == i)
		...
		if(!alreadyVisited)
			potentialWPs[potentialWPs.size] = i;
	}
	
	if(potentialWPs.size > 0)
	{
		chosen = potentialWPs[randomInt(potentialWPs.size)];
		self.prevWaypoints[self.prevWaypoints.size] = chosen;
		if(self.prevWaypoints.size > 3) // Reduced history size
		{
			// Manual array removal - shift all elements left
			for(i = 1; i < self.prevWaypoints.size; i++)
				self.prevWaypoints[i-1] = self.prevWaypoints[i];
	...
	// If all waypoints were visited recently, clear history and pick random
	self.prevWaypoints = [];
	if(level.waypoints.size > 0)
	return randomInt(level.waypoints.size);
	else
		return 0; // Fallback if no waypoints exist
}

findDistantWaypoint()
{
	// Find a waypoint that's far from current position to encourage exploration
	bestDist = 0;
	bestWP = undefined;
	...
	for(i = 0; i < level.waypoints.size; i++)
	{
		...
		// Prefer waypoints that are far away but not too far
		if(dist > 300 && dist < 1200 && dist > bestDist) // Increased minimum distance and maximum range
		{
			// Check if we have any reasonable path
			eye = self.origin + (0, 0, 60);
			trace = bulletTrace(eye, wpOrigin + (0,0,60), false, self);
			
			if(trace["fraction"] > 0.15) // Even more lenient path requirement for exploration
			{
				bestDist = dist;
				bestWP = i;
			}
	...
	// If no distant waypoint found, fall back to random
	if(!isDefined(bestWP))
	{
		if(level.waypoints.size > 0)
			return randomInt(level.waypoints.size);
...
findRandomReachableWaypoint()
{
	// Find a random waypoint that's actually reachable
	eye = self.origin + (0, 0, 60);
	reachableWPs = [];
	
	// Try to find waypoints that are reachable
	for(i = 0; i < level.waypoints.size; i++)
	{
		if(!isDefined(level.waypoints[i]) || !isDefined(level.waypoints[i].origin))
			continue;
		...
		// Only consider waypoints within reasonable distance
		if(dist > 50 && dist < 800)
		{
			// Check if waypoint is reachable
			trace = bulletTrace(eye, wpOrigin + (0,0,60), false, self);
			
			if(trace["fraction"] > 0.6) // Waypoint is reachable
			{
				reachableWPs[reachableWPs.size] = i;
			}
		}
	}
	
	// If we found reachable waypoints, pick one randomly
	if(reachableWPs.size > 0)
	{
		return reachableWPs[randomInt(reachableWPs.size)];
	}
	
	// If no reachable waypoints found, fall back to random
	if(level.waypoints.size > 0)
		return randomInt(level.waypoints.size);
	else
		return 0;
	...
	while (isDefined(self) && isDefined(self.isbot) && self.sessionstate == "playing")
	{
		...
		for(i = 0; i < players.size; i++)
		{
			...
			if(!isDefined(player) || !isAlive(player) || player == self || !isDefined(player.pers["team"]))
				...
				// Zombie bots target hunters (allies team)
			...
			// If enemy is close, attack with melee (increased detection range for zombies)
			if (isDefined(closestEnemy) && closestDist < 300)
			{
				// Clear any waypoint flags to prevent conflicts
				if (isDefined(self.usingEnemyWaypoint))
					self.usingEnemyWaypoint = false;
				if (isDefined(self.currentWaypoint))
					self.currentWaypoint = undefined;
				
				// Add a small delay to prevent rapid switching between behaviors
				wait 0.05;
				
				// Face the enemy
				targetVector = closestEnemy.origin - self.origin;
				targetAngles = vectorToAngles(targetVector);
				self setPlayerAngles(targetAngles);
				
				// Movement logic based on distance to prevent circling
				if (closestDist < 80)
				{
					// Very close - stop moving forward to prevent circling, just melee
					self setWalkDir("none");
				}
				else if (closestDist < 120)
				{
					// Close but not too close - move forward slowly
						self setWalkDir("forward");
					
					// Check for obstacles and avoid them
					eye = self.origin + (0, 0, 50);
					forward = anglesToForward(self getPlayerAngles());
					trace = bulletTrace(eye, eye + (forward[0] * 40, forward[1] * 40, forward[2] * 40), false, self);
					
					if (trace["fraction"] < 0.8) // Path is blocked
					{
						// Try to strafe around obstacles
						right = anglesToRight(self getPlayerAngles());
						traceLeft = bulletTrace(eye, eye + (right[0] * -25, right[1] * -25, right[2] * -25), false, self);
						traceRight = bulletTrace(eye, eye + (right[0] * 25, right[1] * 25, right[2] * 25), false, self);
						
						if (traceLeft["fraction"] > traceRight["fraction"] && traceLeft["fraction"] > 0.6)
						{
							// Strafe left
						self setWalkDir("left");
							wait 0.15;
						}
						else if (traceRight["fraction"] > 0.6)
						{
							// Strafe right
						self setWalkDir("right");
							wait 0.15;
				...
				{
					// Further away - move forward normally
					self setWalkDir("forward");
				}
				
				// Melee attack logic
				if (!isDefined(self.meleeActive))
					self.meleeActive = false;
					
				if (!isDefined(self.meleeStartTime))
					self.meleeStartTime = 0;
					
				currentTime = getTime();
				
				// More aggressive melee cycle for zombies
				if (!self.meleeActive && (currentTime - self.meleeStartTime) > 150)
				{
					self meleeWeapon(true);
					self.meleeActive = true;
					self.meleeStartTime = currentTime;
				}
				else if (self.meleeActive && (currentTime - self.meleeStartTime) > 150)
				{
				...
				// More frequent quick melee when approaching (60% chance)
				if (closestDist < 100 && randomfloat(1) < 0.6)
				{
					self meleeWeapon(true);
					wait 0.03;
					self meleeWeapon(false);
			...
			{
				// No enemies nearby, but zombies should actively seek hunters
				if (isDefined(self.meleeActive) && self.meleeActive)
				{
					self meleeWeapon(false);
					self.meleeActive = false;
				}
				
				// Try to find the nearest enemy to move toward
				nearestEnemy = self findNearestEnemy();
				
				if (isDefined(nearestEnemy))
				{
					// Find a waypoint that leads toward the enemy
					enemyWaypoint = self findWaypointTowardEnemy(nearestEnemy);
					
					if (isDefined(enemyWaypoint))
					{
						// Set up enemy-directed waypoint movement
						self.usingEnemyWaypoint = true;
						self.currentWaypoint = enemyWaypoint;
						self.waypointStuckTime = getTime();
						
						// Move toward the enemy waypoint directly
						if (isDefined(level.waypoints[enemyWaypoint]))
						{
							targetOrigin = level.waypoints[enemyWaypoint].origin;
							dir = targetOrigin - self.origin;
							targetDirection = vectorToAngles(vectorNormalize(dir));
							self setPlayerAngles((0, targetDirection[1], 0));
							...
							self setWalkDir("right");
								...
								{
									// Both sides blocked, clear enemy waypoint and patrol normally
									self.usingEnemyWaypoint = false;
									self bot_patrol_waypoints();
								}
					}
				}
			}
			...
			{
						// No good waypoint found, try direct movement toward enemy
						enemyDir = nearestEnemy.origin - self.origin;
						enemyAngles = vectorToAngles(enemyDir);
						self setPlayerAngles(enemyAngles);
						self setWalkDir("forward");
						
						// Check for obstacles
						eye = self.origin + (0, 0, 50);
						forward = anglesToForward(self getPlayerAngles());
						trace = bulletTrace(eye, eye + (forward[0] * 100, forward[1] * 100, forward[2] * 100), false, self);
						
						if (trace["fraction"] < 0.5) // Path is blocked
						{
							// Try to find any waypoint to get unstuck
							self.currentWaypoint = self findBestWaypoint();
							if (isDefined(self.currentWaypoint))
							{
								self.usingEnemyWaypoint = false;
				...
				{
					// No enemies found at all, but zombies should actively seek hunters
					if (isDefined(self.usingEnemyWaypoint) && self.usingEnemyWaypoint)
					{
						self.usingEnemyWaypoint = false;
					}
					
					// Always try to find the nearest hunter to move toward
					nearestHunter = self findNearestHunter();
					
					if (isDefined(nearestHunter))
					{
						// Move toward the nearest hunter
						targetVector = nearestHunter.origin - self.origin;
						targetAngles = vectorToAngles(targetVector);
						self setPlayerAngles(targetAngles);
						
						// Check if path to hunter is clear
						eye = self.origin + (0, 0, 60);
						forward = anglesToForward(self getPlayerAngles());
						trace = bulletTrace(eye, eye + (forward[0] * 100, forward[1] * 100, forward[2] * 100), false, self);
						
						if (trace["fraction"] >= 0.6) // Path is clear
						{
							self setWalkDir("forward");
						}
						else
						{
							...
							if (traceLeft["fraction"] > traceRight["fraction"] && traceLeft["fraction"] > 0.4)
							{
								self setWalkDir("left");
							}
							else if (traceRight["fraction"] > 0.4)
							{
								self setWalkDir("right");
							}
							else
							{
								// Both blocked, try to find alternative path
								self setWalkDir("none");
								// Try to find a waypoint toward the hunter
								enemyWaypoint = self findWaypointTowardEnemy(nearestHunter);
								if (isDefined(enemyWaypoint))
								{
									self.usingEnemyWaypoint = true;
									self.currentWaypoint = enemyWaypoint;
					...
					{
						// No hunters found, use simple patrol movement
						if (!isDefined(self.lastZombieMoveTime))
							self.lastZombieMoveTime = getTime();
						if (!isDefined(self.zombieDirection))
							self.zombieDirection = randomInt(360);
						if (!isDefined(self.lastZombiePosition))
							self.lastZombiePosition = self.origin;
						
						currentTime = getTime();
						
						// Check if zombie is stuck (not moving)
						if (distance(self.origin, self.lastZombiePosition) < 10)
						{
							// Zombie is stuck, immediately change direction and try to move
							self.zombieDirection = randomInt(360);
							self.lastZombieMoveTime = currentTime;
							self setWalkDir("none");
							
							// Try to move in new direction
							self setPlayerAngles((0, self.zombieDirection, 0));
							eye = self.origin + (0, 0, 60);
							forward = anglesToForward(self getPlayerAngles());
							trace = bulletTrace(eye, eye + (forward[0] * 80, forward[1] * 80, forward[2] * 80), false, self);
							
							if (trace["fraction"] >= 0.4) // Even partially clear path
							{
								self setWalkDir("forward");
							}
							else
							{
								// Still blocked, try strafing
								right = anglesToRight(self getPlayerAngles());
								traceLeft = bulletTrace(eye, eye + (right[0] * -50, right[1] * -50, right[2] * -50), false, self);
								traceRight = bulletTrace(eye, eye + (right[0] * 50, right[1] * 50, right[2] * 50), false, self);
								
								if (traceLeft["fraction"] > traceRight["fraction"] && traceLeft["fraction"] > 0.2)
								{
									self setWalkDir("left");
								}
								else if (traceRight["fraction"] > 0.2)
								{
									self setWalkDir("right");
								}
								else
								{
									// Still stuck, try backing up
									self setWalkDir("back");
								}
							}
						}
						...
						{
							// Zombie is moving, update position
							self.lastZombiePosition = self.origin;
							
							// Change direction every 2-5 seconds
							if (currentTime - self.lastZombieMoveTime > 2000 + randomInt(3000))
							{
								self.zombieDirection = randomInt(360);
								self.lastZombieMoveTime = currentTime;
							}
							
							// Set bot to face current direction
							self setPlayerAngles((0, self.zombieDirection, 0));
							
							// Check if path is clear
							eye = self.origin + (0, 0, 60);
							forward = anglesToForward(self getPlayerAngles());
							trace = bulletTrace(eye, eye + (forward[0] * 60, forward[1] * 60, forward[2] * 60), false, self);
							
							if (trace["fraction"] >= 0.5) // Path is clear
							{
								self setWalkDir("forward");
							}
							else
							{
								...
								if (traceLeft["fraction"] > traceRight["fraction"] && traceLeft["fraction"] > 0.3)
								{
									self setWalkDir("left");
								}
								else if (traceRight["fraction"] > 0.3)
								{
									self setWalkDir("right");
								}
								else
								{
									// Both blocked, immediately change direction
									self.zombieDirection = randomInt(360);
									self.lastZombieMoveTime = currentTime;
									self setWalkDir("none");
								}
		...
		else // Hunter bot behavior 
		{
			// First check for nearby enemies before doing anything else
			closestEnemy = undefined;
			closestDist = 99999;
			...
			for(i = 0; i < players.size; i++)
			{
				...
				// Detect enemies from opposite team
				if((self.pers["team"] == "axis" && player.pers["team"] == "allies") || 
				   (self.pers["team"] == "allies" && player.pers["team"] == "axis"))
				{
					dist = distance(self.origin, player.origin);
					if(dist < closestDist)
					{
						// Hunter bots need line of sight
			...
			// Hunter bots need to detect enemies from the opposite team (zombies)
			if (isDefined(closestEnemy) && closestDist < 1200)
			{
				// Face the enemy
				targetVector = closestEnemy.origin - self.origin;
				targetAngles = vectorToAngles(targetVector);
				...
				// Check if current weapon is a rifle that needs special handling
				currentWeapon = self getCurrentWeapon();
				if (currentWeapon == "kar98k_sniper_mp" || currentWeapon == "kar98k_mp" || currentWeapon == "enfield_mp" || 
					currentWeapon == "springfield_mp" || currentWeapon == "mosin_nagant_mp" || currentWeapon == "mosin_nagant_sniper_mp")
				{
					// For rifles, use a simple single-shot pattern
					if (!isDefined(self.lastRifleShot))
						self.lastRifleShot = 0;
						
					currentTime = getTime();
					
					// Fire rifle every 500ms (simulating bolt-action)
					if ((currentTime - self.lastRifleShot) > 500)
					{
				self fireWeapon(1);
						wait 0.05;
						self fireWeapon(0);
						self.lastRifleShot = currentTime;
					}
				}
				else
				{
					// Other weapons use continuous firing
					self fireWeapon(1);
				}
				
				// Movement and melee logic based on distance
				if (closestDist < 100)
				{
					...
					// Melee cycle: start melee, wait, stop melee, wait
					if (!self.meleeActive && (currentTime - self.meleeStartTime) > 200)
					{
					self meleeWeapon(true);
						self.meleeActive = true;
						self.meleeStartTime = currentTime;
					}
					else if (self.meleeActive && (currentTime - self.meleeStartTime) > 200)
					{
						self meleeWeapon(false);
						self.meleeActive = false;
						self.meleeStartTime = currentTime;
					...
					// Additional quick melee when very close (40% chance)
					if (closestDist < 80 && randomfloat(1) < 0.4)
					{
						self meleeWeapon(true);
						wait 0.05;
						self meleeWeapon(false);
					...
					// Simple movement - just back away occasionally
				...
				{
					// Long range - move forward
					self setWalkDir("forward");
				}
				
				// Crouch occasionally for better accuracy (less frequent)
			...
			{
				// No enemies nearby, stop melee and patrol
				if (isDefined(self.meleeActive) && self.meleeActive)
				{
					self meleeWeapon(false);
					self.meleeActive = false;
				...
				// Set up patrol movement for hunter bots
				self setBotStance("stand");
				self setAim(0);
				self bot_patrol_waypoints();
			}
		...
		// Fallback: if no specific movement was set (no enemies detected or no waypoint movement), patrol normally
		if (!isDefined(self.lastMoveTime))
			self.lastMoveTime = 0;
			
		currentTime = getTime();
		if (currentTime - self.lastMoveTime > 1000) // Every second, ensure we're moving
		{
			// Check if we're not currently moving towards anything specific
			if (!isDefined(self.usingEnemyWaypoint) || !self.usingEnemyWaypoint)
			{
				// If we don't have a current waypoint or it's invalid, find one
				if (!isDefined(self.currentWaypoint) || !isDefined(level.waypoints[self.currentWaypoint]))
				{
					self.currentWaypoint = self findBestWaypoint();
				}
...
// Helper functions for zombie bot pathfinding
findNearestEnemy()
{
	// Returns the nearest enemy player regardless of line of sight
	nearest = undefined;
	minDist = 999999;
...
findNearestHunter()
{
	// Returns the nearest hunter (allies team) that is visible to zombie
	nearest = undefined;
	minDist = 999999;
	...
	for(i = 0; i < players.size; i++)
	{
		...
		if(player.pers["team"] == "allies")
		{
			...
			if(dist < minDist)
			{
				// Check if hunter is visible (line of sight)
				trace = bulletTrace(self.origin + (0,0,50), player.origin + (0,0,50), false, self);
				if(trace["fraction"] > 0.7) // Hunter is visible
				{
					minDist = dist;
					nearest = player;
				}
...
findWaypointTowardEnemy(enemy)
{
	// Find the best waypoint that leads toward the enemy
	if(!isDefined(enemy) || !isDefined(level.waypoints) || level.waypoints.size == 0)
	{
		return undefined;
	}
	...
	enemyDir = enemy.origin - self.origin;
	enemyDir = enemyDir / length(enemyDir); // Normalize
	...
	for(i = 0; i < level.waypoints.size; i++)
	{
		...
		wpDir = wpDir / length(wpDir); // Normalize
		
		// Calculate dot product manually to see how aligned the waypoint is with enemy direction
		alignment = (enemyDir[0] * wpDir[0]) + (enemyDir[1] * wpDir[1]) + (enemyDir[2] * wpDir[2]);
		
		// Distance factor - prefer closer waypoints
		dist = distance(self.origin, level.waypoints[i].origin);
		distFactor = 1.0 / (1.0 + dist / 1000.0); // Normalize distance
		
		// Combined score: alignment + distance factor
		score = alignment + distFactor;
		
		// Prefer waypoints that are closer to the enemy
		enemyDist = distance(level.waypoints[i].origin, enemy.origin);
		if(enemyDist < distance(self.origin, enemy.origin))
			score += 1.0; // Increased bonus for waypoints that get us closer to enemy
		
		// Check if waypoint has line of sight to enemy (bonus)
		wpToEnemyTrace = bulletTrace(level.waypoints[i].origin + (0,0,50), enemy.origin + (0,0,50), false, self);
		if(wpToEnemyTrace["fraction"] > 0.8)
			score += 0.5; // Bonus for waypoints with clear line of sight to enemy
		
		// Check if we can reach this waypoint (no major obstacles)
		selfToWpTrace = bulletTrace(self.origin + (0,0,50), level.waypoints[i].origin + (0,0,50), false, self);
		if(selfToWpTrace["fraction"] > 0.6)
			score += 0.3; // Bonus for reachable waypoints
	...
	// Lower threshold to be more permissive - return waypoint if it has any positive score
	if(bestScore > 0.0)
		return bestWaypoint;
	else
		return undefined;
...
getClosestEnemy()
{
	// Returns the closest alive enemy player to self, or undefined if none found
	closest = undefined;
	minDist = 999999;
...
main()
{
	...
	thread maps\mp\gametypes\maps\_select_map::main();
...
//	level.zom["medkits"] = addCvar( "scr_medkits", 2, 0, 20, "int" );
	...
	level.zom["botamount"] = addCvar( "scr_botamount", 15, 0, 32, "int" );

	// Bot names array
	level.botNames = [];
	level.botNames[level.botNames.size] = "^^22CLI^77.^^22MigueL";
	level.botNames[level.botNames.size] = "^1*^9Para^7noize^1.^5.^3.";
	level.botNames[level.botNames.size] = "^^77x^^88*^^44-^^66LoKKu^^44-";
	level.botNames[level.botNames.size] = "^^55`^^77Ga^^55ng^^77Zap";
	level.botNames[level.botNames.size] = "^^55Elita^^22Kmix^4CZ";
	level.botNames[level.botNames.size] = "^^55ERIKZON";
	level.botNames[level.botNames.size] = "^3MeXicano";
	level.botNames[level.botNames.size] = "^^22G^^99rubase^^22K";
	level.botNames[level.botNames.size] = "^7SZKIELKO^1PL";
	level.botNames[level.botNames.size] = "^6Digi^7Dong";
	level.botNames[level.botNames.size] = "Profi";
	level.botNames[level.botNames.size] = "www.mynl.pl";
	level.botNames[level.botNames.size] = "^5ZDENEK^7[^1CZE^7]";
	level.botNames[level.botNames.size] = "Guayaquil ECU";
	level.botNames[level.botNames.size] = "AdriX";
	level.botNames[level.botNames.size] = "^1VETERAN^4|^7CTF";
	level.botNames[level.botNames.size] = "^^00King$^^33SkiterR";
	level.botNames[level.botNames.size] = "^^44`^^77nL^^44.^^77SkakoL^^44*";
	level.botNames[level.botNames.size] = "^^44`^^77nL^^44.^^77Blanco^^44*";
	level.botNames[level.botNames.size] = "^^44`^^77nL^^44.^^77Dusza^^44*";
	level.botNames[level.botNames.size] = "^^44`^^77nL^^44.^^77Avard^^44*";
	level.botNames[level.botNames.size] = "Klocuch[^312^7]";
	level.botNames[level.botNames.size] = "<J><o><s><h>";
	level.botNames[level.botNames.size] = "^6:::^9Kevin^6:::";
	level.botNames[level.botNames.size] = "^2Yoshi";
	level.botNames[level.botNames.size] = "^7Marcin ^1Najman";
	level.botNames[level.botNames.size] = "^7[^1OUT^7LAW^1]^7Flo^1gger";
	level.botNames[level.botNames.size] = "Andrei [LAT]";
	level.botNames[level.botNames.size] = "^^77I ^^11<3 ^^44nL";
	level.botNames[level.botNames.size] = "^^33Ekwador^^55Manieczki";
	level.botNames[level.botNames.size] = "Selbie";
	level.botNames[level.botNames.size] = "^0WSRH^1PL";
	level.botNames[level.botNames.size] = "^4PIAST^1GKS";
	level.botNames[level.botNames.size] = "Mubarak#22#Libya";
	level.botNames[level.botNames.size] = "^^00matrix99";
	level.botNames[level.botNames.size] = "^5Troll";
	level.botNames[level.botNames.size] = "Ja zw 5 min";
	level.botNames[level.botNames.size] = "eSuba.Lucker";
	level.botNames[level.botNames.size] = "^0Wur^1stc^3hen";
	level.botNames[level.botNames.size] = "^7siadajNaButle^1.^7pl";
	level.botNames[level.botNames.size] = "Our Discord - ds.mynl.pl";
	level.botNames[level.botNames.size] = "AnFe";
	level.botNames[level.botNames.size] = "^^11HancocK";
	level.botNames[level.botNames.size] = "^1|^7MODS^1|^7Fabio";
	level.botNames[level.botNames.size] = "^2Dignity ^1Destroyer";
	level.botNames[level.botNames.size] = "^^00^5^7Veg^9eta^5!?";
	level.botNames[level.botNames.size] = "^^77Typo^^33G";
	level.botNames[level.botNames.size] = "^^88~GALATASARY~";
	level.botNames[level.botNames.size] = "php.";
	level.botNames[level.botNames.size] = "^^77Force 141 ^^22ALCatraz";
	level.botNames[level.botNames.size] = "^^99evil^^11.^^77Faint";
	level.botNames[level.botNames.size] = "^5sniper^1killer";
	level.botNames[level.botNames.size] = "^^99AFK^^7782";
	level.botNames[level.botNames.size] = "United^5#^7Seven";
	level.botNames[level.botNames.size] = "SoulEx";
	level.botNames[level.botNames.size] = "^1!^4S^2.^7C^2.^4U^2.^7D^2.^1~!";
	level.botNames[level.botNames.size] = "^^77K^^221^77nG";
	level.botNames[level.botNames.size] = "^9Muted ^7Wysmarowany";
	level.botNames[level.botNames.size] = "^^11Django";
	level.botNames[level.botNames.size] = "^3Triska.^7ESL";
	level.botNames[level.botNames.size] = "Embrace.";
	level.botNames[level.botNames.size] = "^^11.^7g^94^7ll^9ar^7do^^11";
	level.botNames[level.botNames.size] = "^1|^9GDT^1|^7AdBi";
	level.botNames[level.botNames.size] = "^8Happy";
	level.botNames[level.botNames.size] = "^5TheCROW";
	level.botNames[level.botNames.size] = "^^00^5x^9*_OBEY_";
	level.botNames[level.botNames.size] = "G1_Killer";
	level.botNames[level.botNames.size] = "^^00Woo^^11Doo";
	level.botNames[level.botNames.size] = "^^55x R ^7a i z e n ^^77";
	level.botNames[level.botNames.size] = "Mama Malej Madzi";
	level.botNames[level.botNames.size] = "^17th^7Division LaMa";
	level.botNames[level.botNames.size] = "[P^1L]^7Montgomery";
	level.botNames[level.botNames.size] = "Slaye^4R";
	level.botNames[level.botNames.size] = "^^00somla^^11T";
	level.botNames[level.botNames.size] = "^^22miLo$";
	level.botNames[level.botNames.size] = "^^22Fehvar^^77[HUN]";
	level.botNames[level.botNames.size] = "Ciasny|x|WojteK";
	level.botNames[level.botNames.size] = "^^77Adria^^00N";
	level.botNames[level.botNames.size] = "^^77Rayan^^11PL";
	level.botNames[level.botNames.size] = "Kevi";
	level.botNames[level.botNames.size] = "Bodzio";
	level.botNames[level.botNames.size] = "^2J^7upiter";
	level.botNames[level.botNames.size] = "S^2olid ^7S^2nake";
	level.botNames[level.botNames.size] = "^3Phoenix^1| ^7ephialtes^1.";
	level.botNames[level.botNames.size] = "]^^55R-F^^77[^^00HUEL";
	level.botNames[level.botNames.size] = "^6Dai$y^7<^13";
	level.botNames[level.botNames.size] = "BiggMarketChippy#1987";
	level.botNames[level.botNames.size] = "^^11{D-Company09}^^44Pvt.Ryan";
	level.botNames[level.botNames.size] = "^^00^7M^51^9lf x Str^5i^7ke^5*";
	level.botNames[level.botNames.size] = "^^00^7M^51^9lf x P^5o^7ki^5*";
	level.botNames[level.botNames.size] = "MaraS^1[^7PL^1]";
	level.botNames[level.botNames.size] = "^^11PAT^^33RYCZ^^22ESKO";
	level.botNames[level.botNames.size] = "^^11Sgt.Randall^^44|AD|";
	level.botNames[level.botNames.size] = "^^00D^^11i^^00Zz^^11Y";
	level.botNames[level.botNames.size] = "=[1944]=";
	level.botNames[level.botNames.size] = "CCCP^1.^7DR^1.^4Ce^1ll^7<^4<^1<";
	level.botNames[level.botNames.size] = "^3Tit^4an UK.";
	level.botNames[level.botNames.size] = "^^22ShirO^^66xXx";
	level.botNames[level.botNames.size] = "^1Ncahy";
	level.botNames[level.botNames.size] = "^^99evil^^11.^^77Mi^^22a^^77mi";
	level.botNames[level.botNames.size] = "ManDown (AFK)";
	level.botNames[level.botNames.size] = "^1|XperiA|^7prototype";
	level.botNames[level.botNames.size] = "^5Baranek.Bozzy";
	level.botNames[level.botNames.size] = "Grzegorz x Floryda";
	level.botNames[level.botNames.size] = "This is the Reebok or the Nike";
	level.botNames[level.botNames.size] = "^^66~>s&D<~^^33BelZe:**<3";
	level.botNames[level.botNames.size] = "^^66~>s&D<~^^33Rzeka:**<3";
	level.botNames[level.botNames.size] = "^^66~>s&D<~^^33Box3r:**<3";
	level.botNames[level.botNames.size] = "^5EloN^2MusK^3**";
	level.botNames[level.botNames.size] = "^6`Skillz^9~>^7Snugi";
	level.botNames[level.botNames.size] = "mateusz_1998";
	level.botNames[level.botNames.size] = "^^55Goddess^^66RoXy";
	level.botNames[level.botNames.size] = "^^00Majkel^^11!";
	level.botNames[level.botNames.size] = "Miro Warsztat (nowe konto)";
	level.botNames[level.botNames.size] = "><Zamcio><";
	level.botNames[level.botNames.size] = "^5G^2Force#^1Zero^2Cool";
	level.botNames[level.botNames.size] = "Nem kivant terhesseg";
	level.botNames[level.botNames.size] = "^6majtkomat.pl^7#Krystian";
	level.botNames[level.botNames.size] = "^1ZombieS!^3Diakon";
	...
	level.zom["body_throws"] = addCvar( "scr_body_throws", 2, 0, 5, "int" );
...
startGame()
{
	...
	// Add bots based on cvar
	thread addBotsOnStart();
	...
	thread maps\mp\gametypes\blank::startGame();
	thread teamHud();
	thread globalLogic();
...
//		level.lastManKiller = players[i];
	...
	for(i = 0; i < players.size; i++)
	{
		...
		player maps\mp\gametypes\blank::spawnIntermission();
		player setClientCvar( "cg_objectiveText", text );
	}
	...
	// Kick all bots before disconnecting players
	players = getentarray("player", "classname");
	for(i = 0; i < players.size; i++)
	{
		if(isDefined(players[i].isbot) && players[i].isbot)
		{
			players[i] kickbot();
		}
	}
...
makeZombie( delay )
{
	...
	// Store bot flag before any operations that might lose it
	isBot = isDefined(self.isbot) && self.isbot;

	origin = self getorigin() + (0,0,60);
	angles = self.angles;
	self maps\mp\gametypes\blank::spawnSpectator(origin, angles);
	...
	{
		// For bots, automatically select weapons and spawn
		wait delay;
		// For bots, automatically select random lastman weapon and spawn
		zombieWeapons = [];
		...
		weapon = zombieWeapons[randomInt(zombieWeapons.size)];
		
		// Set the weapon directly for bots to avoid the giveWeapon error
		self.pers["weapon"] = weapon;
		
		// Notify menu response like a player would do
		self notify("menuresponse", game["menu_weapon_axis"], weapon);
		
		wait 0.3;
		maps\mp\gametypes\blank::spawnPlayer();
	}
	else 
	{
		// For human players, show menu as normal
...
teleporter()
{
	...
	self.teleportermodel delete();
	//self maps\mp\gametypes\_util::enableeweapon();
	wait 0.1;
	self.god = false;
}
...
makeLastMan()
{
	...
	if(isDefined(self.isbot))
	{
		// For bots, automatically select random lastman weapon and spawn
		lastmanWeapons = [];
		lastmanWeapons[lastmanWeapons.size] = "super_rocketlauncher_mp";
		...
		// Notify menu response like a player would do
		self notify("menuresponse", game["menu_weapon_lm"], weapon);
		
		wait 0.3;
		
		// Safety check - make sure bot is still valid before spawning
		...
		{
			// Bot is no longer valid, end lastman
			level.lastMan = false;
			level.lastManPicked = false;
		}
	}
...
pickLastManFromScore()
{
	...
	// Safety check - if no hunter found, end lastman
	if (!isDefined(best_hunter))
	{
		level.lastMan = false;
		level.lastManPicked = false;
		return;
...
getBestHunter()
{
	...
	// If no player found, pick random player
	if (!isDefined(guy) && players.size > 0)
	{
...
//	addHud( name, x, y, alignX, alignY, alpha, fontScale, sort, label )
	...
	if( level.zom["hud_hp"] )
		self addTextHud( "health", 75, 459, "left", "middle", 1, 0.7, 10, &"^2" );
...
//#BraXi's Zombies Mod - Sprint function
sprint()
{
	self endon("disconnect");
	self notify( "unbug" );
	...
	self.sprinttime = 0;
	self.sprinting = false;
	self.sprintpower = 128;

	if( isDefined( self.box_sprint ) )
	{
		self.box_sprint destroy();
		self.box_sbackground destroy();
		self.box_sicon destroy();
	}

	self.box_sprint = newclientHudElem(self);
	self.box_sprint.alignX = "left";
	self.box_sprint.alignY = "top";
	self.box_sprint.x = 22;
	self.box_sprint.y = 470;
	self.box_sprint.archived = true;
	self.box_sprint.alpha = 1;
	self.box_sprint.sort = 10;
	self.box_sprint setShader( "gfx/hud/hud@health_bar.dds", 128, 5 ); 
	self.box_sprint.color = (0,.5,1);

	self.box_sbackground = newClientHudElem(self);				
	self.box_sbackground.alignX = "left";
	self.box_sbackground.alignY = "top";
	self.box_sbackground.x = 5;
	self.box_sbackground.y = 468;
	self.box_sbackground.alpha = .7;
	self.box_sbackground.color = (0,0,0);
	self.box_sbackground setShader("gfx/hud/hud@health_bar.dds", 149,10.5);

	self.box_sicon = newClientHudElem(self);				
	self.box_sicon.alignX = "left";
	self.box_sicon.alignY = "top";
	self.box_sicon.x = 8;
	self.box_sicon.y = 470;
	self.box_sicon.sort = 100;
	self.box_sicon.alpha = 1;
	self.box_sicon setShader("gfx/hud/death_crush.dds", 6,6);

	//#Debug - Player's Loop - May Affect
	while( self.sessionstate == "playing" )
	{
		self.oldspeed = self.maxspeed;

		oldorigin = self.origin;
		//iprintln( oldorigin );
		wait 0.1;

		if( !isDefined( self.isInVehicle ) && (oldorigin != self.origin) && self useButtonPressed() && self.sprinttime < self.sprintpower && !self.running && self getStance() == "stand" && !self.isLast ) 
		{	
			x = 128 - (self.sprinttime);
		
			if( isDefined( self.box_sprint ) )
				self.box_sprint setShader( "gfx/hud/hud@health_bar.dds", x, 5 ); 

			//sprint = (float)(128-self.sprinttime)/(float)128;
			//self.box_sprint.color = (1 - sprint, sprint, 1 - sprint);
			...
			if( !self.sprinting )
			{
				...
				self.sprinting = true;
				self.sprinttime = (int)(self.sprinttime);
			}
			else
			{
				// Don't disable weapons for bots during sprint
				if (!isDefined(self.isbot) || !self.isbot)
				self maps\mp\gametypes\_util::disableweapon();
				self setspeed(290);
				self.sprinttime += 1;
			}
		}
		else
		{
			if( self.sprinting )
			{
				// Only enable weapons if they were disabled (not for bots)
				if (!isDefined(self.isbot) || !self.isbot)
				self maps\mp\gametypes\_util::enableweapon();
				self.sprinting = false;
				self setspeed(190);
...
//#BraXi's Zombies Mod - Sprint Regeneration function	
sprint_regeneration()	
{	
	self endon("disconnect");	
	self endon("death");	
	...
	if( self.sprinttime == 128 && isDefined( self.box_sprint ) )	
		self.box_sprint.alpha = 0;	
	wait .3;	
	while( self useButtonPressed() )	
		wait 0.1;	
	while( self.sprinttime != 0 && isDefined( self.box_sprint ) )	
	{		
		if( self.sprinttime < 127 && isDefined( self.box_sprint ) ) self.box_sprint.alpha = 1;	
		self.sprinttime -= .5;
		x = 128 - (self.sprinttime);	
		if( isDefined( self.box_sprint ) )	
			self.box_sprint setShader( "gfx/hud/hud@health_bar.dds", x, 5 );	
...
//	self playLoopSound( "zom_knockout" );
	while( self.knockout && self.sessionstate == "playing" )
	{
		...
		// Don't disable weapons for bots during knockout
		if (!isDefined(self.isbot) || !self.isbot)
		self maps\mp\gametypes\_util::disableweapon();
		wait 0.2;
	}
...
addBotClients()
{
	...
	if( getCvar( "bots" ) == "" ) setCvar( "bots", 0 );
	
	for(;;)
	{
		wait 1;
	
		if( getCvarInt("bots") == 0 )
				continue;
	
		iNumBots = getCvarInt("bots");
...
checkBodySearch(body)
{
	...
	for(;;)
	{
		if( isAlive( self ) && self.sessionstate == "playing" && self meleeButtonPressed() )
		{
			...
			// Don't disable weapons for bots when searching bodies
			if (!isDefined(self.isbot) || !self.isbot)
			self maps\mp\gametypes\_util::disableweapon();
			if(!isdefined(self.body_search_bar))
			{
			...
			if(body.inventory.size && randomInt(20))
			{
				...
				if(found["item"] == "health")
				{
					...
					body maps\mp\gametypes\blank::dropHealth();
				}
				// ammo pack
