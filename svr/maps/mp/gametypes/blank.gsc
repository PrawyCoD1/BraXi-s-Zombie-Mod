...
Callback_PlayerConnect()
{
	...
	self thread codam\_mm_commands::_checkMuted();
	self thread codam\_mm_commands::_checkLoggedIn();
	self thread codam\_mm_commands::_checkFOV();
...
Callback_PlayerDisconnect()
{
	...
	for(i = 0; i < players.size; i++) {
        ...
        if(!isDefined(xtnded) || xtnded == "")
            ...
            xtnded = "1.1x v" + xtnded;
    }

	iprintln(self.name + " Disconnected" + "(" + xtnded + ")" );
	self thread codam\_mm_commands::_delete();
...
Callback_PlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc)
{
	...
	if(self.sessionstate != "dead")
	{
		...
		lpselfGuid = self getIP();
		lpattackerteam = "";

		if(isPlayer(eAttacker))
		{
			lpattacknum = eAttacker getEntityNumber();
			lpattackGuid = eAttacker getIP();
...
spawnPlayer()
{
	...
	// Only give weapon if it's defined to prevent runtime errors
	if(isDefined(self.pers["weapon"]))
	{
...
respawn()
{
	...
	// Special handling for zombie bots to ensure they can always respawn
	if (self.pers["team"] == "axis" && (isDefined(self.isbot) && self.isbot))
	{
		// Zombie bots get immediate respawn without waiting
		wait 0.5;
...
waitRespawnButton()
{
	...
	// Check if this is a bot - if so, auto-respawn without waiting for button press
	if (isDefined(self.isbot) && self.isbot)
	{
		// Bots auto-respawn after a short delay
		wait 0.5;
	...
	// Additional check for zombie bots that might have lost their bot flag
	if (self.pers["team"] == "axis" && !isDefined(self.isbot))
	{
		// This might be a zombie bot that lost its bot flag, give it a chance to respawn
		wait 1.0;
		self notify("remove_respawntext");
		self notify("respawn");
		return;
